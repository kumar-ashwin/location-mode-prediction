...
class TieredEmbedding(nn.Module):
    """
    Embed tiered location as:
    cluster_embed = nn.Embedding(config.total_cluster_num, d_input)
    intra_cluster_embed = nn.Embedding(config.max_intra_cluster_num, d_input)
    emb_proc = Linear(d_input * 2, d_input)
    emb_loc = emb_proc(torch.cat([cluster_embed(cluster), intra_cluster_embed(intra_cluster)], -1))
    """
    def __init__(self, d_input, config) -> None:
        super(TieredEmbedding, self).__init__()
        self.d_input = d_input
        self.cluster_embed = nn.Embedding(config.total_cluster_num, d_input)
        self.intra_cluster_embed = nn.Embedding(config.max_intra_cluster_num, d_input)
        self.emb_proc = nn.Linear(d_input * 2, d_input)

    def forward(self, cluster, intra_cluster) -> Tensor:
        emb_loc = self.emb_proc(torch.cat([self.cluster_embed(cluster), self.intra_cluster_embed(intra_cluster)], -1))
        return emb_loc
...
class AllEmbeddingGeoTron(nn.Module):
    ...
    def forward(self, src, context_dict) -> Tensor:
        ...
        if self.location_mode == "onehot":
            ...
            emb = self.emb_loc(context_dict["cluster"], context_dict["intra_cluster_id"])
        else:
            raise ValueError("Invalid location mode")
